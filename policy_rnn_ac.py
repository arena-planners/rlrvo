"""GRU actor-critic policy from rl_rvo_nav. Vendored verbatim except for a
CUDA-availability guard on the constructor's torch.cuda.synchronize() call so
this loads on CPU-only nodes."""
...
class Critic(nn.Module):
    ...
    def forward(self, obs):
        if self.rnn_reader is not None:
            if isinstance(obs, list):
                obs = self.rnn_reader.obs_rnn_list(obs)
            else:
                obs = self.rnn_reader.obs_rnn(obs)
